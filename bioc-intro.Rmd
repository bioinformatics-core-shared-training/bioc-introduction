...
```{r}
which(features$NCBI.gene.symbol == "BRCA1")
match("BRCA1",features$NCBI.gene.symbol)
grep("BRCA1",features$NCBI.gene.symbol)
grep("BRCA",features$NCBI.gene.symbol)
...
```{r}
cutree(hclust(distMat), h=80)
cutree(hclust(distMat), k=2)
```

We can then compare these new labels to what is already known about the samples

```{r}
mylabels <- cutree(hclust(distMat), h=80)
table(mylabels, metadata$er)
...
- to do this we pick the genes with the highest variance using a handy function

```{r}
library(genefilter)
geneVar <- rowSds(eValues)
topVar <- order(geneVar,decreasing=TRUE)[1:100]
```

Let's just go ahead and run the function

```{r}
heatmap(eValues[topVar,])
```

Cool! There seems to be some kind of structure to the data. But the plot is a little ugly. Fortunately there are plenty of options in the function that we can use to tweak the plot (`?heatmap`)
...
- Modify the row names of the heatmap so the gene names are displayed rather than probe names
...
heatmap(eValues[topVar,],
        ColSideColors = sampColours,
        col=rbPal,
        labRow = features$NCBI.gene.symbol[topVar])
```
